...
export const ForecastComparisonCard = ({
  ...
  const eac =
    metrics.eac ?? (bac > 0 && metrics.cpi ? bac / (metrics.cpi / 100) : 0);
  const ac = metrics.ac ?? 0;
  const pv = metrics.pv ?? 0;
  const vac = metrics.vac ?? bac - eac;
  const etc = metrics.etc ?? eac - ac;

  // Use Forecast-based CPI (Target CPI) as primary, fallback to historical CPI
  const cpiRaw = metrics.cpi_forecast ?? metrics.cpi;
  const cpi =
    cpiRaw !== null
      ? cpiRaw * 100
      : bac > 0 && eac > 0
    ...
    >
      <Row gutter={[16, 16]}>
        ...
        <Col xs={12} sm={8}>
          <Statistic
            title={
              <Tooltip title="Cost Performance Index = BAC / EAC">
                <span>CPI</span>
              </Tooltip>
            }
        ...
        <Col xs={12} sm={8}>
          <Statistic
            ...
            value={
              metrics.spi !== null && metrics.spi !== undefined
                ? Number(metrics.spi).toFixed(2)
                : "N/A"
            }
            valueStyle={{
              color:
                metrics.spi && Number(metrics.spi) > 1
                  ? "#52c41a"
                  : metrics.spi && Number(metrics.spi) < 1
                    ? "#ff4d4f"
                    : "#1890ff",
        ...
        {/* Forecasted PV (New Metric) */}
        <Col xs={12} sm={8}>
          <Statistic
            title={
              <Tooltip title="Forecast value distributed by schedule progression (EAC * Schedule%)">
                <span>PV</span>
              </Tooltip>
            }
            value={pv}
