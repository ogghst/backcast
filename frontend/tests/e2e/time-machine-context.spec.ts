...
test.describe("Time Machine Context Isolation", () => {
  ...
  test("should invalidate all versioned queries when switching branches", async ({
    page,
  }) => {
  ...
  test("should prevent cross-branch data leakage in cost elements", async ({
    page,
  }) => {
    ...
    // Wait for new data to load
    await expect(page.locator('[data-testid="cost-element-name"]')).toBeVisible(
      { timeout: 10000 },
    );

    // Verify data is from the new branch (may be different or not exist)
    const newName = await page
      .locator('[data-testid="cost-element-name"]')
      .textContent();
  ...
  test("should invalidate temporal queries when changing asOf date", async ({
    page,
  }) => {
  ...
  test("should maintain context isolation in WBE hierarchy", async ({
    page,
  }) => {
    ...
    // Get new tree state
    const newTreeNodes = await page
      .locator('[data-testid="wbe-tree-node"]')
      .count();
  ...
  test("should not leak data between contexts when creating entities", async ({
    page,
  }) => {
    ...
    const featureBranchExists = await page
      .locator(
        '[data-testid="branch-option"]:not([data-testid="branch-option-main"])',
      )
      .count();
  ...
  test("should invalidate forecasts when cost data changes", async ({
    page,
  }) => {
    ...
    const initialPV = await page
      .locator('[data-testid="forecast-pv"]')
      .first()
      .textContent();
    ...
    const newPV = await page
      .locator('[data-testid="forecast-pv"]')
      .first()
      .textContent();

    // PV should have increased (new cost element added)
    expect(parseFloat(newPV || "0")).toBeGreaterThan(
      parseFloat(initialPV || "0"),
    );
  });
});

test.describe("Query Key Factory Usage", () => {
  test("should use queryKeys factory for all cache operations", async ({
    page,
  }) => {
    ...
    const hasContextParams = apiRequests.some(
      (r) =>
