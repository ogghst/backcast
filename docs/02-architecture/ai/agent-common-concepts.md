...
flowchart LR
    ...
    WS --> AS["AgentService\n.start_execution()"]
    AS --> ORC{"Orchestrator\nChoice"}
    ORC -->|"use_supervisor=False"| DAO["DeepAgentOrchestrator\n.create_agent()"]
...
When `use_supervisor=True`, `DeepAgentOrchestrator.create_agent()` delegates to `SupervisorOrchestrator.create_supervisor_graph()` instead of building its own subagent-as-tool graph.
