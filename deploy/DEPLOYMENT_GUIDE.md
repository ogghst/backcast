...
#### RustFS (S3) VirtualHost

Routes the Traefik `storage` subdomain to the RustFS object store. Presigned
download URLs (generated by the backend) point here. **`ProxyPreserveHost On` is
required** — RustFS verifies each presigned URL's SigV4 signature against the
`Host` header it receives.

**File:** `/home/nicola/dev/backcast_evs/deploy/apache/storage.backcast.duckdns.org.conf`
...
> The shared certificate (`backcast.duckdns.org.crt`) must cover the `storage`
> subdomain — reissue via certbot adding `-d storage.backcast.duckdns.org` if it
> is not already a SAN.
