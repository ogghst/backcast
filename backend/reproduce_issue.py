...
async def reproduce():
    ...
    try:
        ...
        co_update = ChangeOrderUpdate(
            title="Updated Title 222", description="Updated Description"
        )

        co_id = co.change_order_id

        updated_co = await service.update_change_order(
            change_order_id=co_id, change_order_in=co_update, actor_id=actor_id
        )
        ...
        stmt = text(
            "SELECT id, title, valid_time, transaction_time FROM change_orders WHERE change_order_id = :coid ORDER BY transaction_time"
        )
        ...
        for row in rows:
            print(
                f"ID: {row.id}, Title: {row.title}, Valid: {row.valid_time}, Trans: {row.transaction_time}"
            )
