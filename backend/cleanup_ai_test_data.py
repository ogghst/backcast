...
async def cleanup_ai_test_data():
    ...
    async with async_session_maker() as session:
        ...
        result = await session.execute(
            text(
                "SELECT DISTINCT cost_element_id FROM cost_elements WHERE code LIKE 'CE-%'"
            )
        )
        ...
        if ce_ids:
            ...
            await session.execute(
                text(
                    f"DELETE FROM progress_entries WHERE cost_element_id IN ({placeholders})"
                ),
                params,
            )
            ...
            await session.execute(
                text(
                    f"DELETE FROM cost_registrations WHERE cost_element_id IN ({placeholders})"
                ),
                params,
            )
            ...
            result = await session.execute(
                text(
                    "SELECT DISTINCT forecast_id FROM cost_elements WHERE code LIKE 'CE-%' AND forecast_id IS NOT NULL"
                )
            )
            ...
            await session.execute(
                text(
                    f"DELETE FROM cost_elements WHERE cost_element_id IN ({placeholders})"
                ),
                params,
            )
            ...
            if forecast_ids:
                ...
                await session.execute(
                    text(
                        f"DELETE FROM forecasts WHERE forecast_id IN ({placeholders_f})"
                    ),
                    params_f,
                )
        ...
        result = await session.execute(
            text("DELETE FROM projects WHERE code = 'AI-TEST-001'")
        )

        await session.commit()
        print(
            f"Cleaned up AI tools test data: {len(ce_ids) if ce_ids else 0} cost elements deleted"
        )
