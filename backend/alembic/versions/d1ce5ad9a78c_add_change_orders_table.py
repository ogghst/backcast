...
down_revision: Union[str, Sequence[str], None] = '5ae1f9320c4b'
...
def upgrade() -> None:
    ...
    op.create_table('change_orders',
        sa.Column('change_order_id', postgresql.UUID(), nullable=False, comment='Root UUID identifier for EVCS versioning'),
        sa.Column('code', sa.String(length=50), nullable=False, comment='Business identifier (e.g., CO-2026-001)'),
        sa.Column('project_id', postgresql.UUID(), nullable=False, comment='Reference to the Project this change applies to'),
        sa.Column('title', sa.String(length=200), nullable=False, comment='Brief title of the change'),
        sa.Column('description', sa.Text(), nullable=True, comment='Detailed description of what the change entails'),
        sa.Column('justification', sa.Text(), nullable=True, comment='Business justification for the change'),
        sa.Column('effective_date', postgresql.TIMESTAMP(timezone=True), nullable=True, comment='When the change should take effect'),
        sa.Column('status', sa.String(length=50), nullable=False, server_default='Draft', comment='Workflow state'),
        ...
        sa.Column('branch', sa.String(length=80), nullable=False, server_default='main'),
    ...
    op.create_index(op.f('ix_change_orders_change_order_id'), 'change_orders', ['change_order_id'], unique=False)
    op.create_index(op.f('ix_change_orders_code'), 'change_orders', ['code'], unique=False)
    op.create_index(op.f('ix_change_orders_project_id'), 'change_orders', ['project_id'], unique=False)
    op.create_index(op.f('ix_change_orders_created_by'), 'change_orders', ['created_by'], unique=False)
