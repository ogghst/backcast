...
down_revision: Union[str, Sequence[str], None] = 'd1ce5ad9a78c'
...
def upgrade() -> None:
    ...
    op.create_table(
        'branches',
        sa.Column('name', sa.String(80), nullable=False, comment='Branch name (e.g., main or co-CO-2026-001)'),
        sa.Column('project_id', postgresql.UUID(as_uuid=True), nullable=False, comment='Project this branch belongs to (no FK constraint as project_id is not unique in projects table)'),
        sa.Column('type', sa.String(20), nullable=False, server_default='main', comment='Branch type (main or change_order)'),
        sa.Column('locked', sa.Boolean(), nullable=False, server_default='false', comment='Whether branch is locked (prevents writes)'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('NOW()'), comment='When branch was created'),
        sa.Column('created_by', postgresql.UUID(as_uuid=True), nullable=False, comment='User who created the branch'),
        sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True, comment='Soft delete timestamp'),
        sa.Column('metadata', postgresql.JSONB(), nullable=True, comment='Additional branch information'),
    ...
    op.add_column(
        'change_orders',
        sa.Column('branch_name', sa.String(80), nullable=True, comment='Explicit link to branches table')
    )
