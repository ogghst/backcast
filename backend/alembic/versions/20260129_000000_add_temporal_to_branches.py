...
def upgrade() -> None:
    # Add branch_id
    op.add_column(
        "branches", sa.Column("branch_id", postgresql.UUID(as_uuid=True), nullable=True)
    )  # Nullable first to populate
    ...
    # Create index
    op.create_index(
        op.f("ix_branches_branch_id"), "branches", ["branch_id"], unique=False
    )
