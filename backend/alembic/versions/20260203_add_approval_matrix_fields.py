...
def upgrade() -> None:
    ...
    # Add new columns
    op.add_column(
        "change_orders", sa.Column("impact_level", sa.String(20), nullable=True)
    )
    op.add_column(
        "change_orders",
    ...
    op.add_column(
        "change_orders", sa.Column("sla_status", sa.String(20), nullable=True)
    )
    ...
    op.create_index("ix_change_orders_sla_due_date", "change_orders", ["sla_due_date"])

    # Create index for impact_level to support filtering by impact
    op.create_index("ix_change_orders_impact_level", "change_orders", ["impact_level"])
