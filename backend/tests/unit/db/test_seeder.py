...
@pytest.mark.asyncio
class TestSeedDashboardLayouts:
    """Tests for seed_dashboard_layouts method."""

    async def test_seed_dashboard_layouts_creates_layouts(
        self, db_session: AsyncSession, tmp_path: Path
    ) -> None:
        """Test seeding creates dashboard layouts from seed data."""
        # Create seed file with dashboard layouts
        seed_file = tmp_path / "dashboard_layouts.json"
        layout_data = [
            {
                "id": "01234567-89ab-cdef-0123-456789abcdef",
                "name": "Default EVM Dashboard",
                "description": "Standard Earned Value Management dashboard",
                "user_id": "01234567-89ab-cdef-0123-456789abc00",
                "project_id": None,
                "is_template": True,
                "is_default": True,
                "widgets": [
        ...
        seed_file.write_text(json.dumps(layout_data))

        seeder = DataSeeder(seed_dir=tmp_path)

        # Mock the select statement to return None (layout doesn't exist)
        with patch("app.db.seeder.sql_select"):
            ...
            await seeder.seed_dashboard_layouts(db_session)

            # Verify layout was added to session
            assert db_session.add.called

    async def test_seed_dashboard_layouts_skips_existing(
        self, db_session: AsyncSession, tmp_path: Path
    ) -> None:
        """Test seeding skips existing dashboard layouts."""
        # Create seed file
        seed_file = tmp_path / "dashboard_layouts.json"
        layout_data = [
            {
        ...
        # Mock the select statement to return existing layout
        with patch("app.db.seeder.sql_select"):
            ...
            # Verify layout was NOT added to session
            # (session.add should not be called for existing items)
            assert not db_session.add.called

    async def test_seed_dashboard_layouts_empty_file(
        self, db_session: AsyncSession, tmp_path: Path
    ) -> None:
        """Test seeding with no data."""
        # Create empty seed file
        ...
        # No layouts should be attempted
...
class TestSeedAll:
    ...
    async def test_seed_all_includes_dashboard_layouts(
        self, db_session: AsyncSession, tmp_path: Path
    ) -> None:
        """Test seed_all calls seed_dashboard_layouts."""
        seeder = DataSeeder(seed_dir=tmp_path)

        with patch.object(seeder, "seed_departments"):
            with patch.object(seeder, "seed_users"):
                with patch.object(seeder, "seed_dashboard_layouts") as mock_dash:
                    await seeder.seed_all(db_session)

                    # Verify dashboard layouts was called
                    mock_dash.assert_called_once_with(db_session)
