...
async def test_change_order_workflow_locking_temporal(db_session):
    ...
    # Historical query: check v1 is still unlocked
    branch_v1_historical = await service.branch_service.get_as_of(
        branch_name, project.project_id, 
        as_of=branch_v1.valid_time.lower
    )
    assert branch_v1_historical.locked is False
