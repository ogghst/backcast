...
@pytest.mark.asyncio
async def test_change_order_update_consistency_fix(db_session: AsyncSession, admin_user: User):
    ...
    co = await service.create_change_order(co_create, actor_id=actor_id, control_date=co_create.control_date)
    ...
    co_update = ChangeOrderUpdate(
        title="Updated Title Consistency",
    ...
    stmt = text("SELECT id, valid_time FROM change_orders WHERE change_order_id = :coid")
    ...
    for row in rows:
        ...
        assert valid_time_str != "empty", f"Found version with empty valid_time: {row.id}"
        
        # Also check manually if lower == upper and not empty (though postgres normalizes [x,x) to empty)
        if not row.valid_time.isempty:
            assert row.valid_time.lower != row.valid_time.upper, f"Found version with zero duration: {row.id}"
...
async def test_change_order_active_versions(db_session: AsyncSession, admin_user: User):
    ...
    co = await service.create_change_order(
        ChangeOrderCreate(code=f"CO-ACT-{uuid4().hex[:4]}", project_id=project.project_id, title="V1"),
        actor_id=actor_id
    )
...
async def test_change_order_crud_lifecycle(db_session: AsyncSession, admin_user: User):
    ...
    update_in = ChangeOrderUpdate(description="Updated Desc", status="Submitted for Approval")
