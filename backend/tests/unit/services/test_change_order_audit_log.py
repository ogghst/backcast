...
class TestChangeOrderAuditLog:
    ...
    @pytest.mark.asyncio
    async def test_status_transition_with_comment_succeeds(self, db_session: AsyncSession):
        ...
        updated_co = await service.update_change_order(
            co.change_order_id, co_update, actor_id=uuid4()
        )
    ...
    @pytest.mark.asyncio
    async def test_status_transition_without_comment_succeeds(self, db_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_multiple_status_transitions_create_audit_trail(self, db_session: AsyncSession):
        ...
        await service.update_change_order(
            co.change_order_id,
            ChangeOrderUpdate(status="Submitted for Approval", comment="Ready for review"),
            actor_id=actor_id,
        )
