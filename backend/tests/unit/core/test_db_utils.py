...
@pytest.mark.asyncio
async def test_safe_db_execute_success(async_session: AsyncSession):
    """Test safe_db_execute with successful query."""

    async def mock_query():
        return await async_session.execute(text("SELECT 1"))

    result = await safe_db_execute(async_session, mock_query(), "Test query")
...
async def test_safe_db_execute_with_error(async_session: AsyncSession):
    ...
    with pytest.raises(ValueError, match="Failed query"):
        await safe_db_execute(async_session, failing_query(), "Failed query")
