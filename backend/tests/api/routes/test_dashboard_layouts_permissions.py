"""Phase 6 — Backend persistence correctness for global dashboard widgets.

Covers four gaps from
``docs/03-project-plan/iterations/2026-06-29-global-dashboard-widgets/
global-dashboard-widgets-design.md`` (§7 Phase 6):
...
- **G13** — `get_templates(scope=...)` filters templates by ``global`` /
  ``project`` scope.
...
# ---------------------------------------------------------------------------
# G13 — get_templates scope filter
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_get_templates_scope_filter(db: AsyncSession) -> None:
    """``get_templates(scope=...)`` filters by global vs project scope.

    - ``scope="global"`` -> only ``project_id IS NULL``
    - ``scope="project"`` -> only ``project_id IS NOT NULL``
    ...
    try:
        ...
        # seeded global templates; filter by name prefix to reason deterministically).
        ...
        assert ours_all == {"TPL-Global", "TPL-Project"}

        ours_global = await names("global")
        assert ours_global == {"TPL-Global"}
        ...
        assert ours_unknown == {"TPL-Global", "TPL-Project"}
    finally:
        for c in created:
