...
class MockRBACService(RBACServiceABC):
    ...
    def get_user_permissions(self, user_role: str) -> list[str]:
        return [
            ...
            "cost-element-update",
            "cost-element-delete",
            "project-create", "wbe-create", "department-create", "cost-element-type-create", # Dependencies
        ]
...
async def setup_dependencies(client: AsyncClient) -> dict[str, Any]:
    ...
    # 1. Department
    dept_res = await client.post("/api/v1/departments", json={"code": f"D-{uuid4().hex[:4].upper()}", "name": "Dept"})
    dept_id = dept_res.json()["department_id"]
    
    # 2. Cost Element Type
    type_res = await client.post("/api/v1/cost-element-types", json={"code": f"T-{uuid4().hex[:4].upper()}", "name": "Type", "department_id": dept_id})
    type_id = type_res.json()["cost_element_type_id"]
    
    # 3. Project
    proj_res = await client.post("/api/v1/projects", json={"code": f"P-{uuid4().hex[:4].upper()}", "name": "Proj", "budget": 100})
    proj_id = proj_res.json()["project_id"]
    
    # 4. WBE
    wbe_res = await client.post("/api/v1/wbes", json={"code": f"W-{uuid4().hex[:4].upper()}", "name": "WBE", "project_id": proj_id})
...
@pytest.mark.asyncio
async def test_create_cost_element(client: AsyncClient, setup_dependencies: dict[str, Any]) -> None:
...
@pytest.mark.asyncio
async def test_create_cost_element_in_branch(client: AsyncClient, setup_dependencies: dict[str, Any]) -> None:
    deps = setup_dependencies
    element_data = {
    ...
    response = await client.post("/api/v1/cost-elements?branch=feature-1", json=element_data)
    assert response.status_code == 201
    data = response.json()
    assert data["branch"] == "feature-1"

@pytest.mark.asyncio
async def test_update_forks_branch(client: AsyncClient, setup_dependencies: dict[str, Any]) -> None:
    deps = setup_dependencies
    # 1. Create in main
    element_data = {
    ...
    create_res = await client.post("/api/v1/cost-elements?branch=main", json=element_data)
    ...
    response = await client.put(
        f"/api/v1/cost-elements/{element_id}?branch=feature-fork",
        json=update_data
    )
...
@pytest.mark.asyncio
async def test_list_filtering(client: AsyncClient, setup_dependencies: dict[str, Any]) -> None:
    deps = setup_dependencies
    # Create 2 elements
    await client.post(
        "/api/v1/cost-elements",
        json={
            "code": "CE-1", "name": "E1", "budget_amount": 100,
            "wbe_id": deps["wbe_id"], "cost_element_type_id": deps["cost_element_type_id"]
        }
    )
...
@pytest.mark.asyncio
async def test_delete_cost_element_branch(client: AsyncClient, setup_dependencies: dict[str, Any]) -> None:
    deps = setup_dependencies
    element_data = {
