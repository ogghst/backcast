...
class TestBriefingWithPlan:
    ...
    def test_scoped_markdown_filters_sections(self) -> None:
        """Only sections matching relevant_step_indices are included."""
        doc = BriefingDocument(
            original_request="Analyze project",
            sections=[
                BriefingSection(
                    specialist_name="project_manager",
                    findings="Listed projects",
                    step_index=0,
                ),
                BriefingSection(
                    specialist_name="evm_analyst",
        ...
        md = doc.to_scoped_markdown(relevant_step_indices={2, 0})
        assert "Listed projects" in md
        assert "CPI=0.85" not in md
        assert "Chart built" in md

    def test_scoped_markdown_includes_no_step_sections(self) -> None:
        """Sections with step_index=None are always included."""
        doc = BriefingDocument(
            original_request="test",
            sections=[
                BriefingSection(
                ...
                BriefingSection(
                    specialist_name="evm_analyst",
                    findings="Step 1 finding",
                    step_index=1,
                ),
            ],
        )
        md = doc.to_scoped_markdown(relevant_step_indices={1})
        assert "Generic finding" in md
        assert "Step 1 finding" in md

    def test_scoped_markdown_omits_execution_plan(self) -> None:
        """Execution Plan section is never rendered in scoped output."""
        doc = BriefingDocument(
            original_request="test",
            plan=[{"specialist": "evm_analyst", "task_description": "calc"}],
        )
        md = doc.to_scoped_markdown(relevant_step_indices={0})
        assert "## Execution Plan" not in md

    def test_scoped_markdown_none_returns_full(self) -> None:
        """Passing None falls back to full to_markdown()."""
        doc = BriefingDocument(
            original_request="test",
            plan=[{"specialist": "evm_analyst", "task_description": "calc"}],
        )
        md = doc.to_scoped_markdown(relevant_step_indices=None)
        assert "## Execution Plan" in md

    def test_scoped_markdown_keeps_request_and_analysis(self) -> None:
        """Request header and supervisor analysis are always included."""
        doc = BriefingDocument(
            original_request="Do the thing",
            supervisor_analysis="Complex request needing 3 steps",
        )
        md = doc.to_scoped_markdown(relevant_step_indices={0})
        assert "## Request" in md
        assert "Do the thing" in md
        assert "## Supervisor Analysis" in md
        assert "Complex request" in md
