...
There you can use the `fastapi run --reload` command to run the debug live reloading server. The default port is 8010 (set via PORT environment variable).

```console
$ fastapi run --reload --port 8010 app/main.py
```
...
```console
root@7f2607af31c3:/app# fastapi run --reload --port 8010 app/main.py
```
