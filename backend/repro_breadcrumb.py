...
async def repro():
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL",
        "postgresql+asyncpg://postgres:postgres@localhost:5432/backcast_evs",
    )
    ...
    async with async_session() as session:
        ...
        try:
            breadcrumb = await service.get_breadcrumb(
                wbe_id, branch="main", branch_mode=BranchMode.MERGE
            )
            ...
            breadcrumb_l1 = await service.get_breadcrumb(
                l1_id, branch="main", branch_mode=BranchMode.MERGE
            )
