...
class DataSeeder:
    ...
    async def seed_cost_element_types(self, session: AsyncSession) -> None:
        ...
        for _, item in enumerate(ce_type_data):
            try:
                ...
                if dept_code not in dept_cache:
                    ...
                    if not dept:
                        logger.warning(
                            f"Department {dept_code} not found needed for {item.get('code')}"
                        )
                        skipped_count += 1
                        continue
        ...
        for _, item in enumerate(wbe_data):
            try:
                ...
                if p_code not in proj_cache:
                    ...
                    if not proj:
                        logger.warning(
                            f"Project {p_code} not found for WBE {item['code']}, skipping"
                        )
                        skipped_count += 1
                        continue
                    proj_cache[p_code] = (
                        proj.project_id
                    )  # ID needed for WBE.project_id is ID (root ID)
                ...
                if parent_code:
                    if parent_code not in wbe_cache:
                        ...
                        logger.warning(
                            f"Parent {parent_code} not found in cache for {item['code']}. Assuming missing or order issue."
                        )
