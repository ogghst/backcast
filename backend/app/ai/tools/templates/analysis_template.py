...
async def calculate_evm_metrics(
    ...
    evm_data = await service.calculate_evm(
    ...
    return {
        ...
        "planned_value": float(evm_data.planned_value) if evm_data.planned_value else 0.0,
        "earned_value": float(evm_data.earned_value) if evm_data.earned_value else 0.0,
        "actual_cost": float(evm_data.actual_cost) if evm_data.actual_cost else 0.0,
        "cost_variance": float(evm_data.cost_variance) if evm_data.cost_variance else 0.0,
        "schedule_variance": float(evm_data.schedule_variance) if evm_data.schedule_variance else 0.0,
        "cost_performance_index": float(evm_data.cpi) if evm_data.cpi else 0.0,
        "schedule_performance_index": float(evm_data.spi) if evm_data.spi else 0.0,
        "variance_at_completion": float(evm_data.vac) if evm_data.vac else 0.0,
        "estimate_to_complete": float(evm_data.etc) if evm_data.etc else 0.0,
        "estimate_at_completion": float(evm_data.eac) if evm_data.eac else 0.0,
    }
...
async def get_evm_performance_summary(
    ...
    Returns:
        Dictionary with performance summary and assessment
    ...
    # Get EVM metrics
    evm_data = await service.calculate_evm(project_id=UUID(project_id))

    # Determine performance status
    cpi = float(evm_data.cpi) if evm_data.cpi else 0.0
    spi = float(evm_data.spi) if evm_data.spi else 0.0
...
async def analyze_cost_variance(
    ...
    Returns:
        Dictionary with detailed cost variance analysis
    ...
    variance_data = await service.analyze_cost_variance(project_id=UUID(project_id))

    # Convert to AI-friendly format
    return {
        "project_id": project_id,
        "total_variance": float(variance_data.total_variance) if variance_data.total_variance else 0.0,
        "variance_percentage": variance_data.variance_percentage if variance_data.variance_percentage else 0.0,
        "variance_by_wbe": [
            {
                "wbe_id": str(v.wbe_id),
                "wbe_name": v.wbe_name,
                "variance": float(v.variance) if v.variance else 0.0,
                "variance_percentage": v.variance_percentage if v.variance_percentage else 0.0,
            }
            for v in variance_data.by_wbe
        ] if variance_data.by_wbe else [],
        "root_causes": variance_data.root_causes if variance_data.root_causes else [],
    }
...
async def analyze_schedule_variance(
    ...
    Returns:
        Dictionary with detailed schedule variance analysis
    ...
    variance_data = await service.analyze_schedule_variance(project_id=UUID(project_id))

    # Convert to AI-friendly format
    return {
        "project_id": project_id,
        "total_variance_days": variance_data.total_variance_days if variance_data.total_variance_days else 0,
        "critical_path_delay_days": variance_data.critical_path_delay_days if variance_data.critical_path_delay_days else 0,
        "delayed_activities": [
            {
                "activity_id": str(a.activity_id),
                "activity_name": a.activity_name,
                "delay_days": a.delay_days if a.delay_days else 0,
            }
            for a in variance_data.delayed_activities
        ] if variance_data.delayed_activities else [],
        "schedule_risks": variance_data.schedule_risks if variance_data.schedule_risks else [],
    }
...
async def generate_project_forecast(
    ...
    return {
        ...
        "estimated_final_cost": float(forecast.estimated_final_cost) if forecast.estimated_final_cost else 0.0,
        "estimated_completion_date": forecast.estimated_completion_date.isoformat() if forecast.estimated_completion_date else None,
        "cost_variance_at_completion": float(forecast.vac) if forecast.vac else 0.0,
        "schedule_variance_days": forecast.schedule_variance_days if forecast.schedule_variance_days else 0,
        "confidence_level": forecast.confidence_level if forecast.confidence_level else "Medium",
    }
...
async def compare_forecast_scenarios(
    ...
    Returns:
        Dictionary comparing different forecast methods
...
async def get_forecast_accuracy(
    ...
    Returns:
        Dictionary with forecast accuracy metrics
    ...
    # Get forecast accuracy metrics
    accuracy = await service.get_forecast_accuracy(project_id=UUID(project_id))

    # Convert to AI-friendly format
    return {
        "project_id": project_id,
        "mean_absolute_percentage_error": accuracy.mape if accuracy.mape else 0.0,
        "mean_absolute_error": float(accuracy.mae) if accuracy.mae else 0.0,
        "forecast_bias": accuracy.bias if accuracy.bias else 0.0,
        "recommendation": "Forecasts are reliable" if accuracy.mape and accuracy.mape < 10 else "Forecasts have high uncertainty",
    }
...
async def get_project_kpis(
    ...
    Returns:
        Dictionary with KPIs and health assessment
    ...
    return {
        ...
        "kpis": {
            ...
            "cost_variance": round(float(evm_data.cost_variance) if evm_data.cost_variance else 0.0, 2),
            "schedule_variance": round(float(evm_data.schedule_variance) if evm_data.schedule_variance else 0.0, 2),
        },
        "recommendations": [
