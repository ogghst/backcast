...
class SupervisorOrchestrator:
    ...
    def _create_specialist_wrapper(
        ...
        async def specialist_node(
            ...
            # When a plan exists but has no matching step, the supervisor's
            # explicit handoff overrides the plan — the specialist should run
            # (task_description comes from the handoff via briefing.task_history).
            completed = state.get("completed_specialists", set())
            ...
            invocation_id = state.get("current_invocation_id", "") or str(
                uuid.uuid4()
            )
            ...
            if self._event_bus is not None:
                ...
                if success_briefing_md:
                    completed_set = cmd_update.get(
                        "completed_specialists", set()
                    )
                    completed_list = sorted(completed_set) if isinstance(
                        completed_set, set
                    ) else []
                    self._event_bus.publish(
                        AgentEvent(
                ...
                # Publish the specialist's text findings as a token_batch so
                # they appear in the specialist's chat bubble.  The specialist
                # runs via ainvoke (non-streaming), so findings arrive as a
                # single batch after completion.
                if cleaned_findings:
                    self._event_bus.publish(
                        AgentEvent(
                            ...
                            data={
                                "type": "token_batch",
                                "tokens": cleaned_findings,
                                "session_id": "",
                                "source": "subagent",
                                "subagent_name": specialist_name,
                                "invocation_id": invocation_id,
                            },
                            timestamp=datetime.now(UTC),
                        )
